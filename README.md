...
| `check-invariants.py` | CNAME 等、消えると壊れるものが消えていないか／共通ヘッダーの CSS がページ側に複製されていないか／`<title>` が規約に合っているか（`_tools/seo/title-policy.md`） |
...
### `<title>` は規約に合っていれば、それ以上議論しない（2026-08-14 追加）

正本は `_tools/seo/title-policy.md`、登録簿は `_tools/seo/title-registry.json`。
**規約・登録簿・検査の3点で1組**なので、どれか1つだけ直さない。

タイトルは「毎回レビューで指摘され、毎回直すのに終わらない」状態が続いていた。原因は回帰では
なく**完了の定義が無かったこと**で、公開64ページでサフィックスが9種類・区切りが3種類に割れて
いた（この状態では誰が見ても何か指摘できる）。規約の本体は次の3行:

- **規約に適合しているタイトルは、レビューで議論しない**
- **勝ちページは `frozen`。型に吸収しない**（レビュー対象外であって、未処理の負債ではない）
- **記事型をカタログ型に強制しない**

登録簿は `frozen`（期限なし・成果が出ているので触らない）と `migration`（期限付き・いつか直す）
を別の状態として持つ。`migration` はページが規約に適合したら検査が「名簿から消すこと」と報告
するが、`frozen` は適合していても残す。

**タイトルの抽出に正規表現を使わない。** `trial-program/index.html` の冒頭コメントに
`- <title>を確定版に更新` があり、`<title>(.*?)</title>` がコメント側から拾って壊れた文字列を
返していた。`contamination()` もそれを「空ではない」と判定して通していた。現在は
`hublib.page_title()`（`html.parser` で `<head>` 内のみ）に統一している。

2026-08 時点では**レポートモード**（違反を報告するが公開は止めない）。公開停止への昇格と
公開タイトルの移行は 9/11 以降（検索流入の立ち上がりと外部掲載の余波を避けるため）。
...
検査はインライン `<style>` に加えて、ページが読み込む**ローカルの CSS ファイルも見る**
（外部 CSS へ移して逃げられないようにするため）。`https://` などの外部 URL は読まない。

公開30ページの移行は 2026-08-14 に完了し、`check-invariants.py` の
`HEADER_CSS_LEGACY`（移行中だけ通す名簿）は空になっている。**ここに新しいページを
足さないこと** — 足したくなったら、それは直すべき複製の方。

カタログの製品ページは ②' の `_brochure_poc/build_pages.py` が生成する。
**生成器の `TEMPLATE` にもヘッダー CSS を書かない**（書くと全カタログページへ複製が配られる）。
